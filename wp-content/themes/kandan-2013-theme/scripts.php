...
function pageLoader(){
	...
	$('#page-loader').css({'height': pageLoaderHeight});
...
function detectDevices(){
	var deviceAgent = navigator.userAgent.toLowerCase();
    	...
    	if (agentID) {
		$('#intro-slider').css({'background-image':'none'});
...
function resizethis(){
	...
	$('#intro-slider').css({'height': theheight});
}

/* ORGANISE PAGE CONTENT */

function organiseContent(){
	$('#landing_page .content-section li:last').css({'margin-right':'0'});
	$('#people-listing_page li:nth-child(5n)').css({'margin-right':'0'});
}
...
$(document).ready(function(){
	$('#social-media-icons a').hover(function(){
		$(this).animate({'margin-top':'-5px', 'opacity':'1'},300);
		}, function(){
			$(this).animate({'margin-top':'0', 'opacity':'0.8'},100);
		}
	);
});
...
$(document).ready(function(){
	...
	$('.services-icons a').hover(function(){
		$(this).animate({ backgroundColor :'#d5d4d4;'},{queue: false, duration: 300});
		}, function(){
			$(this).animate({ backgroundColor :'#e9e8e8;'},300);
		}
	);
...
$('.hover-arrows').each(function(){
	$(this).parent('a').parent('li').hover(function(){
		$(this).find('.people-title').css({'color' :'#0077ac'});
		$(this).find('.people-name').css({'color' :'#0077ac'});
		$(this).find('.hover-arrows').fadeOut();
		$(this).find('.hover-arrows-hover').fadeIn();
	}, function(){
		$(this).find('.people-title').css({'color' :'#000'});
		$(this).find('.people-name').css({'color':'#000'});
...
$('#people-listing_page #content .left-column li').each(function(){
	$(this).hover(function(){
		$(this).find('.serious-face').animate({'opacity' :'0'});
	}, function(){
		$(this).find('.serious-face').animate({'opacity' :'1'});
	});
});
});
...
// FOOTER COOKIE FOR CONTACT DETAILS 
	...
	if (closeContacts == 'yes'){
		...
		$('#footer_address').css({'display':'none'});
	}
