...
/*
// All click events to fire the loader graphic.
// All excpet the link with a data bypass data attribute
// And the slider click events on the home page, next, prev, and small bullets
// and all the post inserted badly by the not so clever worpress people.
*/
...
/*
// hides the loader of you click on it. just incase page gets stuck.
//
*/
$(".loader").click(function() {
    hideloader();
});
$(".close-cross-btn").click(function() {
    hideloader();
...
/*
//Add Active class to navigation tabs for appropriate menu item.

*/

$(document).ready(function() {
    var url = window.location;
    // // boolean for checking if active already added.
    // var added = false;
    // add class if you can find it?
    ...
    var count = 0;
    $('ul.nav a').parent().each(function(index, evt) {

        if ($(this).hasClass('active')) {
            // increment 1 if 1 or more has class of active
            count++;
        }
    })
    ...
    if (count > 0) {
        // if already added then no nav class needed.
        // end script
        return false;
    } else {
        // ok so no page is active.... now find out who is left.
        // /either work, people, or blog.
        var pathArray = url.pathname.split('/');
        // get name of where the person is.
        var folder_structure = pathArray[1];
        //console.log(folder_structure);

        // APPLY WHERE NECESSARY as urls are no consistent
        switch (folder_structure) {

            case 'the_work':
                $('[data-number="3"]').addClass('active');
                break;

            case 'the_people':
                $('[data-number="5"]').addClass('active');
                break;

            case '':
                // $('[data-number="5"]').addClass('active');
                break;
                // BLOG URLS are not obvious so we have to rely on this being the very lasy option.
            default:
                console.log(folder_structure);
                $('[data-number="4"]').addClass('active');
                break;
        }
...
function menuArrowSwitch() {
    /* MOVES ARROW */
    $('#primary_nav ul li a').not('.see_more').not('.read_more').toggle(
        function() {
            $(this).parent().css({
...
/* WHY - because the desktop site is so pixel perfect... we need to remove the width and height attributes to make them fluid for mobile */

function removeWHAttr() {
    ...
    $('#people-listing_page ul li a').each(function() {
        $(this).children('img').removeAttr('width');
        $(this).children('img').removeAttr('height');
    });

    $('.people_page-img').each(function() {
        $(this).children('img').removeAttr('width');
        $(this).children('img').removeAttr('height');
    });

    $('#the_work_page #content p a').each(function() {
        $(this).children('img').removeAttr('width');
        $(this).children('img').removeAttr('height');
    });
